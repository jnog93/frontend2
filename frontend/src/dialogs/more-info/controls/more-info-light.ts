...
class MoreInfoLight extends LitElement {
  ...
  render(): TemplateResult {
    ...
    //var borderRadius = this.stateObj?.borderRadius.borderRadius ? this.stateObj.borderRadius : '12px';
    var borderRadius = '12px';

    //var supportBrightness = this.stateObj?.attributes.brightness;
    ...
    if (this.stateObj.attributes.supported_color_modes[0] === "hs") {
      actions = [{ "action": "call-service", "service": "light.turn_on", "service_data": { "entity_id": entity, "rgb_color": [3, 0, 255] }, "color": "#0300ff" },
      { "action": "call-service", "service": "light.turn_on", "service_data": { "entity_id": entity, "rgb_color": [255, 136, 0] }, "color": "#ff8800" },
      { "action": "call-service", "service": "light.turn_on", "service_data": { "entity_id": entity, "rgb_color": [40, 255, 0] }, "color": "#28ff00" }
      ]

    }
    else if (this.stateObj.attributes.supported_color_modes[0] === "brightness"){
      actions = [{ "action": "call-service", "service": "light.turn_on", "service_data": { "entity_id": entity, "brightness": 85 }, "color": "#DCDCDC" },
      { "action": "call-service", "service": "light.turn_on", "service_data": { "entity_id": entity, "brightness": 190 }, "color": "#F0F0F0" },
        { "action": "call-service", "service": "light.turn_on", "service_data": { "entity_id": entity, "brightness": 255 }, "color": "#FFFFFF" }
      ]
...
var switchValue = 0;
    ...
    var color = this._getColorForLightEntity(this.stateObj, undefined, undefined);
    var sliderColor = "#FFF";
    ...
    //   }
    // }
    var brightness = this.stateObj?.attributes.brightness ? Math.round(this.stateObj.attributes.brightness / 2.55) : 0;

    console.log("offStates", offStates.includes(this.stateObj.state))
    return html`
      <div class="${fullscreen === true ? 'popup-wrapper':''}">
            <div id="popup" class="popup-inner">
                ${hideIcon ? html`` : html`
                <div class="icon${fullscreen === true ? ' fullscreen':''}">
                    <ha-icon style="${onStates.includes(this.stateObj.state) ? 'color:'+color+';' : ''}" icon="${icon}" />
                </div>
                `}
                ${((this.stateObj.attributes.supported_color_modes[0] != 'onoff') && displayType == 'auto') || (displayType == 'slider') ? html`
                    ${hideState ? html`` : html`<h4 id="brightnessValue">${offStates.includes(this.stateObj.state) ? this.hass.localize(`component.light.state._.off`) : brightness + '%'}</h4>`}
                    <div class="range-holder" style="--slider-height: ${brightnessHeight};--slider-width: ${brightnessWidth};">
                        <input type="range" style="--slider-width: ${brightnessWidth};--slider-height: ${brightnessHeight}; --slider-border-radius: ${borderRadius};${sliderColoredByLight ? '--slider-color:'+color+';':'--slider-color:'+sliderColor+';'}--slider-thumb-color:${sliderThumbColor};--slider-track-color:${sliderTrackColor};" .value="${offStates.includes(this.stateObj.state) ? 0 : Math.round(this.stateObj.attributes.brightness/2.55)}" @input=${e => this._previewBrightness(e.target.value)} @change=${e => this._setBrightness(this.stateObj, e.target.value)}>
                    </div>
                ` : html`
                    ${hideState ? html`` : html`<h4 id="switchValue">${computeStateDisplay(this.hass.localize, this.stateObj, this.hass.language)}</h4>`}
                    <div class="switch-holder" style="--switch-height: ${switchHeight};--switch-width: ${switchWidth};">
                        <input type="range" style="--switch-width: ${switchWidth};--switch-height: ${switchHeight}; --slider-border-radius: ${borderRadius}; --switch-color: ${switchColor}; --switch-track-color: ${switchTrackColor};" value="0" min="0" max="1" .value="${switchValue}" @change=${() => this._switch(this.stateObj)}>
                    </div>
                `}
                ...
                ${this.settings ? html`<button class="settings-btn ${this.settingsPosition}${fullscreen === true ? ' fullscreen':''}" @click="${() => this._openSettings()}">${'Settings'}</button>`:html``}
            </div>

            ${this.settings ? html`
              <div id="settings" class="settings-inner" @click="${e => this._close(e)}">
                ${this.settingsCustomCard ? html`
                  <div class="custom-card" data-card="${'entities'}" data-options="${JSON.stringify({'entities': ['light.raceland']})}" data-style="${''}">
                  </div>
                `: html`

                    <more-info-light2 .stateObj = ${this.stateObj} .hass = ${this.hass}></more-info-light2>
                `}
                <button class="settings-btn ${this.settingsPosition}${fullscreen === true ? ' fullscreen':''}" @click="${() => this._closeSettings()}">${'Close'}</button>
              </div>
            `:html``}
        </div>
    `;
  ...
  _openSettings() {
    console.log("opensetting", this.shadowRoot?.getElementById('settings'));
...
// customElements.define('more-infor-light', MoreInfoLight);
